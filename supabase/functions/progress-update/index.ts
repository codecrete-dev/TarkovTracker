...
serve(async (req) => {
  ...
  try {
    ...
    if (!body.progressData || typeof body.progressData !== "object") {
      return createErrorResponse("Missing or invalid progressData", 400)
    }
    ...
    if (progressData.level !== undefined) {
      if (typeof progressData.level !== "number" || progressData.level < 1 || progressData.level > 100) {
        return createErrorResponse("Level must be a number between 1 and 100", 400)
      }
    }
    ...
    if (progressData.faction !== undefined) {
      ...
      if (!validFactions.includes(progressData.faction)) {
        return createErrorResponse(`Faction must be one of: ${validFactions.join(", ")}`, 400)
      }
    }
    ...
    if (fetchError && fetchError.code !== "PGRST116") {
      ...
      return createErrorResponse("Failed to fetch current progress", 500)
    }
