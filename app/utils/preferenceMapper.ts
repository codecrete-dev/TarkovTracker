export type FlatPreferences = Record<string, any>;
...
const COLUMN_TO_PROPERTY_MAP: Record<string, string> = Object.entries(PROPERTY_TO_COLUMN_MAP).reduce(
  (acc, [prop, col]) => ({ ...acc, [col]: prop }),
  {}
);
 ...
 */
export function flattenPreferences(nested: any): FlatPreferences {
  ...
  const recurse = (obj: any, currentPath: string) => {
    for (const key in obj) {
      ...
      if (value !== null && typeof value === 'object' && !Array.isArray(value) && key !== 'individualHide') {
        recurse(value, newPath);
      } else {
        const columnName = PROPERTY_TO_COLUMN_MAP[newPath] || key.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`);
 ...
 */
export function unflattenPreferences(flat: FlatPreferences, defaultState: any): any {
