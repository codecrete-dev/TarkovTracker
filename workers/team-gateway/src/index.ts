export interface Env {
  TEAM_GATEWAY_LIMITER: DurableObjectNamespace;
  SUPABASE_URL: string;
...
export class TeamGatewayRateLimiter {
...
async function rateLimit(
  ...
  const id = env.TEAM_GATEWAY_LIMITER.idFromName(key);
  const stub = env.TEAM_GATEWAY_LIMITER.get(id);
...
async function handleAction(request: Request, env: Env, action: TeamAction) {
  ...
  const rl = await rateLimit(env, key, limit, windowSec);
  if (!rl.allowed) {
    return new Response(rl.message || 'Rate limit exceeded', {
  ...
  if (action === 'team-leave') {
    ...
    if (membership.role === 'owner') {
      ...
      const delMemberships = await api(`team_memberships?team_id=eq.${teamId}`, {
        method: 'DELETE',
      });
      ...
      const delMem = await api(`team_memberships?team_id=eq.${teamId}&user_id=eq.${userId}`, {
        method: 'DELETE',
      });
      if (!delMem.ok) {
...
async function handleTokenAction(request: Request, env: Env, action: TokenAction) {
  ...
  const rl = await rateLimit(env, rlKey, limit, windowSec);
  if (!rl.allowed) {
    return new Response(rl.message || 'Rate limit exceeded', {
      status: rl.status || 429,
      headers,
  ...
  if (action === 'token-create') {
    ...
    if (!gameMode || permissions.length === 0) {
      return jsonResponse(
        { error: 'gameMode and permissions are required' },
        400,
        origin,
        reqOrigin
      );
    ...
    if (!resp.ok) {
      ...
      try {
        ...
        if (errJson?.code === '42703') {
          ...
          return jsonResponse(
            { success: true, tokenId: JSON.parse(text)[0]?.token_id, tokenValue },
            200,
            origin,
            reqOrigin
          );
    ...
    return jsonResponse(
      { success: true, tokenId: created?.token_id, tokenValue },
      200,
      origin,
      reqOrigin
    );
